...
class LiveChannelManager(
        ...
        val envelope =
            LiveChannelEnvelope(
                ...
                sender =
                    LiveChannelSender(
                        nodeId = proxyId,
                        nodeType = LiveChannelNodeType.VELOCITY,
                    ),
        ...
        channelSubscriptions
            ...
            .forEach { (expectedPayloadType, subscriptions) ->
                if (expectedPayloadType != envelope.payloadType) {
                    logger.warn(
                        "Dropped live channel payload due to type mismatch: channel={}, senderNodeId={}, senderNodeType={}, expectedPayloadType={}, actualPayloadType={}, subscriberCount={}",
                        envelope.channelName,
                        envelope.sender.nodeId,
                        envelope.sender.nodeType,
                        expectedPayloadType,
    ...
    private fun decodePayload(
        ...
        try {
            ...
            logger.warn(
                "Dropped live channel payload due to decode failure: channel={}, senderNodeId={}, senderNodeType={}, payloadType={}",
                envelope.channelName,
                envelope.sender.nodeId,
                envelope.sender.nodeType,
                envelope.payloadType,
    ...
    private fun deliverPayload(
        ...
        try {
            ...
            logger.error(
                "Live channel listener failed: channel={}, senderNodeId={}, senderNodeType={}, payloadType={}, subscriptionId={}",
                envelope.channelName,
                envelope.sender.nodeId,
                envelope.sender.nodeType,
                envelope.payloadType,
