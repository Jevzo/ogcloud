...
class RedisManager(
    ...
    fun getServerData(id: String): RedisServerData? {
        return readValue(serverKey(id), RedisServerData::class.java)
    }

    fun getServerIds(): Set<String> {
        return readMembers(SERVERS_KEY)
    }

    fun getServerIdsByGroup(group: String): Set<String> {
        return readMembers(groupServersKey(group))
    }

    fun getPlayerData(uuid: String): RedisPlayerSession? {
        return readValue(playerKey(uuid), RedisPlayerSession::class.java)
    }
    ...
    private fun <T> readValue(key: String, type: Class<T>): T? {
    ...
    private fun <T> deserialize(key: String, json: String, type: Class<T>): T? {
        return try {
    ...
    private fun RedisServerData.isTransferEligible(): Boolean {
        return state == RUNNING_STATE && type != PROXY_TYPE
    }
