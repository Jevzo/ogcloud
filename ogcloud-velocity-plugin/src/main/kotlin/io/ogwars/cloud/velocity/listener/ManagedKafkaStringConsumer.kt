...
internal class ManagedKafkaStringConsumer(
    ...
    private fun runConsumerLoop() {
        ...
        try {
            ...
            if (running.get()) {
                logger.error("{} consumer was interrupted unexpectedly", consumerLabel)
            }
        } catch (exception: Exception) {
            logger.error("$consumerLabel consumer thread crashed", exception)
        } finally {
            consumer = null
    ...
    private fun processRecord(payload: String) {
        try {
            ...
            logger.error("Failed to process $consumerLabel event", exception)
        }
    }
