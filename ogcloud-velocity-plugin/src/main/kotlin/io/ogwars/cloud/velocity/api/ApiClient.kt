...
data class ApiMotdSettings(
    val global: String,
    val maintenance: String,
)

data class ApiVersionNameSettings(
    val global: String,
    val maintenance: String,
)
...
class ApiClient(
    ...
    fun getServer(id: String): CompletableFuture<ApiServerResponse> =
        get("/api/v1/servers/$id", ApiServerResponse::class.java)
    ...
    fun stopServer(id: String): CompletableFuture<Void> = post("/api/v1/servers/$id/stop", null)

    fun killServer(id: String): CompletableFuture<Void> = post("/api/v1/servers/$id/kill", null)
    ...
    fun listGroups(): CompletableFuture<List<ApiGroupResponse>> =
        getAllPaged("/api/v1/groups", object : TypeToken<ApiPaginatedResponse<ApiGroupResponse>>() {}.type)

    fun getGroup(id: String): CompletableFuture<ApiGroupResponse> =
        get("/api/v1/groups/$id", ApiGroupResponse::class.java)

    fun setGroupMaintenance(
        id: String,
        enabled: Boolean,
    ): CompletableFuture<Void> = put("/api/v1/groups/$id/maintenance", mapOf("maintenance" to enabled))
    ...
    fun getPlayer(uuid: String): CompletableFuture<ApiPlayerResponse> =
        get("/api/v1/players/$uuid", ApiPlayerResponse::class.java)

    fun setPlayerGroup(
        uuid: String,
        group: String,
        duration: String,
    ): CompletableFuture<ApiPlayerResponse> =
        putWithResponse(
    ...
    fun getNetworkSettings(): CompletableFuture<ApiNetworkSettingsResponse> =
        get("/api/v1/network", ApiNetworkSettingsResponse::class.java)
    ...
    fun executeCommand(
        ...
        post("/api/v1/command", mapOf("target" to target, "targetType" to targetType, "command" to command))
    ...
    private fun post(
        ...
        send(buildJsonRequest(path, body, HttpMethod.POST)).thenApply {
            null
        }
    ...
    private fun put(
        ...
        send(buildJsonRequest(path, body, HttpMethod.PUT)).thenApply {
            null
        }
    ...
    private fun <T> putWithResponse(
        ...
        send(buildJsonRequest(path, body, HttpMethod.PUT)).thenApply { response ->
            gson.fromJson(
                response.body(),
                type,
            )
    ...
    private fun encodeQueryParam(value: String): String = encode(value)

    private fun withPageParams(
        path: String,
        page: Int,
    ): String {
    ...
    private fun requestBuilder(
        ...
        HttpRequest.newBuilder().uri(URI.create("$base$path")).timeout(REQUEST_TIMEOUT).apply {
            if (authorize) {
