...
object PlayerCommands {
    ...
    private fun findPlayer(ctx: CommandContext<CommandSource>, apiClient: ApiClient): Int {
        val source = ctx.source
        val name = ctx.getArgument("name", String::class.java)

        apiClient.listOnlinePlayers(name = name).thenAccept { players ->
            if (players.isEmpty()) {
    ...
    private fun listPlayers(
        ...
        val serverId = if (serverInput != null) {
        ...
        apiClient.listOnlinePlayers(serverId = serverId).thenAccept { players ->
            if (players.isEmpty()) {
    ...
    private fun transferPlayer(ctx: CommandContext<CommandSource>, apiClient: ApiClient): Int {
        val source = ctx.source
        val name = ctx.getArgument("name", String::class.java)
        val target = ctx.getArgument("target", String::class.java)

        apiClient.listOnlinePlayers(name = name).thenCompose { players ->
            if (players.isEmpty()) {
                OgCloudCommand.sendError(source, "Player '$name' not found online.")
                return@thenCompose java.util.concurrent.CompletableFuture.completedFuture(null)
            }
