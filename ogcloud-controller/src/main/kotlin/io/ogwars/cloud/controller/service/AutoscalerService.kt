...
class AutoscalerService(
    ...
    private fun evaluateProxyGroup(
        group: GroupDocument,
    ...
    private fun evaluateDynamicGroup(
        group: GroupDocument,
        servers: List<ServerDocument>,
    ) {
        if (shouldSkipScaling(group)) return
        ...
        if (ratio > group.scaling.scaleUpThreshold && totalActive < group.scaling.maxInstances) {
            log.info(
                ...
                group.id,
                ratio,
                group.scaling.scaleUpThreshold,
            )
    ...
    private fun evaluateStaticGroup(
        group: GroupDocument,
        servers: List<ServerDocument>,
    ) {
    ...
    private fun evaluateGroupSafely(
        group: GroupDocument,
    ...
    private fun shouldSkipScaling(group: GroupDocument): Boolean = group.maintenance || isOnCooldown(group)
    ...
    private fun isPendingServer(server: ServerDocument): Boolean = server.state in PENDING_SERVER_STATES

    private fun recordScaleAction(groupId: String) {
        lastScaleAction[groupId] = Instant.now()
    }

    private fun calculateLoadRatio(
        totalPlayers: Int,
        runningCount: Int,
        group: GroupDocument,
    ): Double? {
    ...
    private fun handleProxyScaleDown(
        ...
        log.info(
            ...
            ratio,
            group.scaling.scaleDownThreshold,
        )
    ...
    private fun handleDynamicScaleDown(
        ...
        emptyLobbies
            .take(minOf(excessCount, emptyLobbies.size))
            .forEach { server ->
                log.info(
    ...
    private fun requestServerForScaling(
        group: GroupDocument,
        reason: String,
        details: String,
    ) {
    ...
    private fun drainServerForScaling(
    ...
    private fun undrainServerForScaling(
        server: ServerDocument,
        reason: String,
        details: String,
    ) {
    ...
    companion object {
        ...
        private val PENDING_SERVER_STATES =
            setOf(
