...
object PaginationSupport {
    ...
    fun resolvePageSize(size: Int?): Int {
        return (size ?: DEFAULT_PAGE_SIZE).coerceAtMost(MAX_PAGE_SIZE)
    }

    fun toPageRequest(page: Int, size: Int?): PageRequest {
        return PageRequest.of(page, resolvePageSize(size))
    }

    fun <T> toResponse(items: List<T>, page: Int, size: Int, totalItems: Long): PaginatedResponse<T> {
    ...
    fun <T> paginate(items: List<T>, page: Int, size: Int?): PaginatedResponse<T> {
    ...
    fun buildSearchCriteria(query: String?, vararg fields: String): Criteria? {
    ...
    private fun normalizeQuery(query: String?): String? {
        return query?.trim()?.takeUnless(String::isEmpty)
    }

    private fun calculateSafeOffset(page: Int, resolvedSize: Int, totalItems: Int): Int {
