...
export const NETWORK_MOTD_MAX_LINES = 2;

const capitalizeWord = (value: string) =>
    value.charAt(0).toUpperCase() + value.slice(1).toLowerCase();

export const countTextLines = (value: string) => value.split(/\r?\n/).length;

export const truncateTextToMaxLines = (value: string, maxLines: number) =>
    value.split(/\r?\n/).slice(0, maxLines).join("\n");

export const createRestartConfirmationCode = () => `${Math.floor(100000 + Math.random() * 900000)}`;
...
export const formatNetworkLockType = (type: string) =>
    type.split("_").map(capitalizeWord).join(" ");
